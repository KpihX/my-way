...
function MapEvents({ points, selectedPlaces, setPoints, max_points, setSelectedPlaces }) {
  ...
  const addPoint = point => {
    ...
    const newPlace = {
      lat: point.lat,
      lon: point.lng,
      display_name: `Place: ${point.lat}, ${point.lng}`,
      address: {
        type: 'point',
        importance: 0.5,
        place_rank: 30,
...
const Map = () => {
  ...
  return (
    <div>
      ...
      <MapContainer center={yaounde} zoom={13} style={{ height: '400px', zIndex: 10 }}>
        ...
        {points.map((point, idx) => (
          <Marker
            key={idx}
            position={point}
            icon={idx === 0 ? marquerIcon : idx === points.length - 1 ? endMarquer : middleMarquer}
            draggable={true}
            eventHandlers={{
              dragend(event) {
          ...
          >
            <Popup autoClose={false} closeOnClick={false} closeButton={false}>
              {idx === 0 ? 'Départ' : idx === points.length - 1 ? 'Destination' : `Escale ${idx}`}
            </Popup>
            <Tooltip>{idx === 0 ? 'Départ' : idx === points.length - 1 ? 'Destination' : `Escale ${idx}`}</Tooltip>
          </Marker>
        ))}
        <MapEvents
          points={points}
      ...
      <button onClick={() => setPoints(points.slice(0, -1))}>Retirer le dernier marqueur</button>
      <button onClick={() => setPoints([])}>Rétirer tous les marqueurs</button>
