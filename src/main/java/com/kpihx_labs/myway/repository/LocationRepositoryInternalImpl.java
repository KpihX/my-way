...
import com.kpihx_labs.myway.repository.rowmapper.LocationRowMapper;
...
class LocationRepositoryInternalImpl extends SimpleR2dbcRepository<Location, Long> implements LocationRepositoryInternal {
    ...
    private static final Table categoryTable = Table.aliased("location_category", "category");
    ...
    RowsFetchSpec<Location> createQuery(Pageable pageable, Condition whereClause) {
        ...
        columns.addAll(LocationCategorySqlHelper.getColumns(categoryTable, "category"));
    ...
    private Location process(Row row, RowMetadata metadata) {
        ...
        entity.setCategory(locationcategoryMapper.apply(row, "category"));
        return entity;
    }
